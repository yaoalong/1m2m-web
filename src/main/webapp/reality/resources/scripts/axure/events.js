...
//        var y = JSON.stringify(axEventObject);
    ...
    var _initializeObjectEvents = function (query, allowItem) {
        ...
        query.each(function (dObj, elementId) {
            ...
            var needsMouseFilter = dObj.type != 'hyperlink' && !$ax.public.fn.IsLayer(dObj.type) && !$ax.public.fn.IsDynamicPanel(dObj.type) && dObj.type != 'richTextPanel' && !$ax.public.fn.IsRepeater(dObj.type) && !$ax.public.fn.IsCheckBox(dObj.type) && !$ax.public.fn.IsRadioButton(dObj.type) && !$ax.public.fn.IsTreeNodeObject(dObj.type);
            if (needsMouseFilter) {
